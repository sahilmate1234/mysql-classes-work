...
-- Q44
SELECT ProductName, Price, GetDiscountedPrice(Price) AS DiscountedPrice FROM Products;

-- Q45
DELIMITER //
CREATE FUNCTION TotalOrdersByCustomer(cid INT)
RETURNS INT
DETERMINISTIC
BEGIN
    DECLARE total INT;
    SELECT COUNT(*) INTO total FROM Orders WHERE CustomerID = cid;
    RETURN total;
END //
DELIMITER ;
